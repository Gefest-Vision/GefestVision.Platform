...
public class TwinController : ControllerBase
{
    ...
    [SwaggerResponseHeader(
        new[] { (int)HttpStatusCode.OK, (int)HttpStatusCode.BadRequest, (int)HttpStatusCode.InternalServerError },
    ...
    public async Task<IActionResult> ReportTwinProperties([FromRoute] string deviceId,
        [Required] [FromBody] Dictionary<string, string> reportedProperties)
    {
    ...
    [SwaggerResponseHeader(
        ...
        "X-Transaction-Id", "string",
        "The header that has the transaction ID is used to correlate multiple operation calls.")]
    public async Task<IActionResult> TwinChangedNotification([FromRoute] string deviceId,
        [Required] [FromBody] TwinInformation twinInformation)
    {
        try
        {
